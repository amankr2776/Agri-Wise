      ...
      <div className="lg:col-span-2 space-y-6">
        <Card className="border-none shadow-xl rounded-[2.5rem] p-8 bg-white relative overflow-hidden">
          ...
          <CardHeader className="p-0 mb-8 relative z-10">
            ...
            <CardDescription className="text-muted-foreground font-medium italic">Multimodal Precision Engine • Bhashini ASR Bridge</CardDescription>
          </CardHeader>
          ...
          <CardContent className="p-0 space-y-8 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-6">
                ...
                <div className="space-y-2 relative">
                  ...
                  <Textarea
                    placeholder="Describe specific spots, curls, or behavior..."
                    value={symptoms}
                  ...
                  <Button 
                    ...
                    className={cn("absolute bottom-4 right-4 h-12 w-12 rounded-xl shadow-lg transition-all", isListening && "animate-pulse")}
                  >
            ...
            <Button 
              className="w-full h-16 text-xl font-black bg-primary hover:bg-primary/90 rounded-[2rem] shadow-xl transition-all active:scale-[0.98]"
              disabled={loading}
          ...
          <Card className={cn(
            ...
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <h3 className="text-3xl font-black tracking-tight text-slate-900">{result.pathogenIdentification}</h3>
                  {result.isBotanicallyValid ? (
                    <Badge className="bg-primary/10 text-primary border-none px-3 font-black text-[9px] uppercase">Precision Analysis</Badge>
                  ) : (
                    <Badge variant="destructive" className="animate-pulse px-3 font-black text-[9px] uppercase gap-1">
                      <AlertTriangle className="h-3 w-3" /> Generic Mode
                    </Badge>
                  )}
                </div>
                <p className="text-muted-foreground font-medium italic">Identification logic for {cropType} family</p>
              </div>
              <Button 
                variant="ghost" 
                size="icon" 
                className={cn("h-14 w-14 rounded-2xl bg-muted hover:bg-primary transition-all", isSpeaking && "bg-primary text-white animate-pulse")}
            ...
            <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 space-y-3">
              <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                <BrainCircuit className="h-4 w-4" /> AI Reasoning (Steps 1-3)
              </h4>
              ...
              <div className="p-8 rounded-[2.5rem] bg-destructive/5 border border-destructive/10 space-y-4">
                ...
                <ul className="space-y-3">
                  {result.suggestedChemicalRemedies.map((rem, i) => (
                    <li key={i} className="text-lg font-bold text-slate-900 leading-tight">• {rem}</li>
                  ))}
                </ul>
              </div>
              ...
              <div className="p-8 rounded-[2.5rem] bg-primary/5 border border-primary/10 space-y-4">
                ...
                <ul className="space-y-3">
                  {result.suggestedTraditionalRemedies.map((rem, i) => (
                    <li key={i} className="text-lg font-medium text-slate-700 italic leading-tight">"{rem}"</li>
                  ))}
                </ul>
              </div>
            </div>
            ...
            <div className="pt-8 border-t flex flex-col items-center gap-6">
              {!result.isBotanicallyValid && (
                <div className="flex items-center gap-3 text-sm text-amber-600 bg-amber-50 p-4 rounded-2xl border border-amber-100 font-medium">
                  ...
                  Botanical precision alert: AI reasoning is broad-spectrum. Requesting Human Scientist review.
                </div>
              )}
              <Button 
                ...
                className={cn(
                  ...
                  isSentToExpert ? "bg-green-50 text-green-600 border border-green-200" : "bg-slate-900 text-white hover:bg-slate-800 shadow-xl"
                )}
              >
                <UserCheck className="h-5 w-5" /> 
                {isSentToExpert ? "Scientist Notified" : "Certify Logic via Scientist"}
              </Button>
            </div>
          </Card>
        )}
        ...
        <Card className="border-none shadow-xl rounded-[2.5rem] p-8 bg-slate-900 text-white">
          <CardHeader className="p-0 mb-6">
            <CardTitle className="text-[10px] font-black flex items-center gap-2 text-primary uppercase tracking-widest">
              <BrainCircuit className="h-4 w-4" /> Neural Node Status
            </CardTitle>
          </CardHeader>
          <div className="space-y-8">
            <div className="p-6 rounded-3xl bg-white/5 border border-white/10 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-black uppercase tracking-widest">Active Hub: 02</span>
              </div>
              <p className="text-xs text-slate-400 font-medium leading-relaxed italic">
                "Gemini 2.5 Flash node processing botanical metadata. Multi-step cross-referencing active for {language}."
              </p>
            </div>
          </div>
        </Card>
